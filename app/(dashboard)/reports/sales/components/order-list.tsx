...
import { SalesReportOrder } from '@/lib/types';
import {
  cn,
  formatCurrency,
...
export const OrderList = ({ data, isLoading }: OrderListProps) => {
  return (
    <div className="flex flex-col gap-1 text-xs">
      <h2 className="text-base font-medium">Daftar Transaksi Penjualan</h2>
      <Separator />
      {isLoading &&
        Array.from({ length: 5 }).map((_, index) => (
          <div
            key={index}
            className="animate-pulse bg-gray-200 h-10 w-full rounded-xs mb-2"
          />
        ))}

      {!isLoading && (data || []).length === 0 && (
        <p className="text-gray-400">Tidak ada transaksi penjualan.</p>
      )}
      ...
      {!isLoading &&
        ...
        data.map(order => {
          ...
          if (isReceivable) {
            totalPaid = parseFloat(payment_cash) + parseFloat(payment_non_cash);
          }
