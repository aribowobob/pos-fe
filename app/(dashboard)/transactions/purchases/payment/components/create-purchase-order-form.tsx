...
interface CreatePurchaseOrderFormProps {
  ...
  isLoading: boolean;
  createOrder: (order: CreatePurchasesOrderRequest) => void;
}
...
export const CreatePurchaseOrderForm = ({
  ...
  const [payableNotif, setPayableNotif] = useState(false);

  const generateOrderNumber = () => {
    const now = new Date();
    const timestamp = now.getTime().toString().slice(-6);
    return `TRB${timestamp}`;
  };
  ...
  const handleCreateOrder = () => {
    ...
    if (totalPayment < cartSummary.grandTotal && !payableNotif) {
      setPayableNotif(true);
      return;
    }

    const order: CreatePurchasesOrderRequest = {
  ...
  const totalPayment =
    parseFloat(paymentCash || '0') + parseFloat(paymentNonCash || '0');
  const change = totalPayment - cartSummary.grandTotal;
  const payable = cartSummary.grandTotal - totalPayment;
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-4">
        ...
        {/* Order Number */}
        <div className="space-y-2">
          <Label htmlFor="order-number">Nomor Pembelian</Label>
          <Input
            id="order-number"
            value={orderNumber}
        ...
        <div className="border rounded-sm p-3 bg-blue-50 space-y-2 col-span-2">
          ...
          {totalPayment < cartSummary.grandTotal ? (
            <p className="text-xs text-red-600 mt-1">
              Pembayaran kurang {formatCurrency(payable)}
            </p>
          ) : (
      ...
      <AlertDialog open={payableNotif} onOpenChange={setPayableNotif}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Pembayaran kurang!</AlertDialogTitle>
            <AlertDialogDescription>
              Kamu yakin ingin melanjutkan dengan pembayaran kurang{' '}
              {formatCurrency(payable)}. Kekurangan pembayaran akan ditandai
              sebagai hutang.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
